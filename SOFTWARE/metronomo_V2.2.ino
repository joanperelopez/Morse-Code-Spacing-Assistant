/*
  CW Visual Metronome - Edit CHAR and FARM speeds
  ...
  - LED character (char gap)         : D9
  - LED word (word gap)              : D10
...
/*
  CW Visual Metronome - Edit CHAR and FARM speeds (fixed encoder delta)
  Arduino Nano - pinout según PCB de Pere

  - Keyer input (active LOW)         : D2
  - Rotary encoder A/B               : D3, D4
  - LCD 1602 (4-bit)                 : D5, D6, D7(RS), D8(E), D11, D12
  - LED character (char gap)         : D9
  - LED word (word gap)              : D10
  - Option button (active LOW)       : A2  (internal pull-up)
  - EEPROM:
      addr 0 -> wpm_char (5..35) default 25
      addr 1 -> wpm_eff  (5..35) default 6

  Nota: se usa encoder.getPosition() y delta = pos - lastPos para detectar movimiento.
*/
...
const uint8_t PIN_LED_CHAR = 9;
const uint8_t PIN_LED_WORD = 10;
...
void debug_final() {
Serial.print("charGap_ms = ");
Serial.print(charGap_ms);
Serial.print("   wordGap_ms = ");
Serial.print(wordGap_ms);

Serial.print("   elapsed = ");
Serial.println(elapsed_p);
}
...
void computeFarnsworthTiming(int wpm_char, int wpm_eff) {
  ...
  t_char_sec = 3.0 * t_slot;
  t_word_sec = 7.0 * t_slot;
...
void showNormal() {
  ...
  showLEDTimes(); // se muestra al inicio
}
...
//  Serial.begin(115200);   // <-- añade esto
  lcd.begin(16,2);
